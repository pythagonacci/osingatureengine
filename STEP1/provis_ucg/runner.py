...
from .otel import get_tracer, init_tracing
...
def run(
    repo_path: str, include_vendor=False, include_minified=False, include_generated=False
) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        ...
        "anomalies": [
            asdict(a) | {"typ": a.typ.name, "severity": a.severity.value} for a in result.anomalies
        ],
...
def main() -> None:
    ...
    parser.add_argument(
        "--include-vendor", action="store_true", help="Include vendor dirs (still flagged)"
    )
    parser.add_argument(
        "--include-minified", action="store_true", help="Include minified JS (still flagged)"
    )
    parser.add_argument(
        "--include-generated", action="store_true", help="Include generated files (still flagged)"
    )
