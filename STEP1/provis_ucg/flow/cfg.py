...
from .model import CFGBlock, CFGBlockKind, CFGEdge, FlowResult, DEFAULT_BOUNDS
...
def _scan_tokens_js(code: str, start_byte: int) -> List[_Token]:
    ...
    while i < L:
        ...
        if in_template:
            if ch == "`":
                in_template = False; i += 1; continue
            # skip ${ ... } nesting crudely
            if code.startswith("${", i):
                depth = 1; i += 2
                while i < L and depth > 0:
                    if code.startswith("${", i): depth += 1; i += 2; continue
                    if code[i] == "}": depth -= 1; i += 1; continue
                    i += 1
                continue
            i += 1; continue
        if in_str:
...
def build_cfg_for_function(
    ...
    *,
    ...
    # Token scan
    toks = _scan_tokens_py(code, sp.byte_start) if language == Language.PYTHON else _scan_tokens_js(code, sp.byte_start)

    # Seed blocks
    entry = CFGBlock(block_id=_bid(fn_node.node_id, 0), fn_node_id=fn_node.node_id, kind=CFGBlockKind.ENTRY, spans=[sp])
    exitb = CFGBlock(block_id=_bid(fn_node.node_id, 1), fn_node_id=fn_node.node_id, kind=CFGBlockKind.EXIT, spans=[sp])
    ...
    for t in toks:
        ...
        b = CFGBlock(block_id=_bid(fn_node.node_id, cur_idx), fn_node_id=fn_node.node_id,
        ...
        edges.append(CFGEdge(
            ...
            cond_label=None,
            spans=b.spans,
            reason_label=f"cfg:seq",
        ))
        last_block_id = b.block_id

        # Add labeled edges for specific constructs
        w = t.text
        if w in ("if", "elif", "else"):
            label = w
        elif w in ("case", "default"):
            label = w
        elif w in ("except", "catch"):
            label = w
        else:
            label = None
