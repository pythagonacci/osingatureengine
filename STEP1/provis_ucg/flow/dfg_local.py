...
from .model import DEFAULT_BOUNDS, DFGFact, DFGOpKind, FlowResult
...
def build_ssa_lite_for_function(
    ...
    *,
        ...
        if len(facts) >= DEFAULT_BOUNDS.max_facts:
            anomalies.append(
                Anomaly(
                    path=sp.path,
                    blob_sha256="",
                    typ=AnomalyType.CONST_FOLD_CUTOFF,
            ...
            return
        fspan = Span(
            sp.path, sp.byte_start + byte_off, sp.byte_start + byte_off, 0, 0, 0, 0, reason
        )
        facts.append(
            DFGFact(
    ...
    # Simple normalizers
    def fold_literal(expr: str) -> tuple[str | None, list[str]]:
        flags: list[str] = []
        expr = expr.strip()
        # string literal?
    ...
    def normalize_call(expr: str) -> tuple[str | None, list[str], str]:
        e = expr.strip()
        flags: list[str] = []
    ...
    for idx, raw in enumerate(lines):
        ...
        if not _is_pathlike_name(name):
            ...
            if not any(tok in name.lower() for tok in _PATH_TOKENS) and not re.search(
                r"(URL|URI|HOST|PATH|ROUTE|SQL|TABLE)", name
            ):
                continue
        ...
        if norm is not None:
            ...
            add_fact(
                src=f"lit:{norm[:64]}",
                dst=f"var:{name}",
                op=DFGOpKind.CALL_NORM,
                byte_off=raw.find(name),
                flags=flags,
                reason=why,
            )
            continue
        ...
        if val is not None:
            ...
            add_fact(
                ...
                op=DFGOpKind.ASSIGN,
                byte_off=raw.find(name),
                flags=(["FOLDED"] + flags if flags else ["FOLDED"]),
    ...
    for n in all_nodes:
        ...
        is_effect = any(
            tok in low
            for tok in (_PY_EFFECT_FUNCS if language == Language.PYTHON else _JS_EFFECT_FUNCS)
        )
        if not is_effect:
            continue
        ...
        for varname, sval in env.items():
            if varname in prev:
                add_fact(
                    src=f"var:{varname}",
                    dst=f"call@{csp.byte_start}",
                    op=DFGOpKind.USE,
                    byte_off=csp.byte_start,
                    flags=[],
                    reason="use:effect_arg",
                )
