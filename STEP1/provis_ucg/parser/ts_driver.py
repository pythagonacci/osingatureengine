...
from tree_sitter import Language as TS_Language, Parser as TS_Parser, Query, Node
...
def _load_language(language: Language) -> TS_Language:
    ...
    raise RuntimeError("Tree-sitter grammars not found. Install 'tree_sitter_languages' or set TS_LANG_BUNDLE.")
...
def _detect_file_info(path: Path) -> Tuple[bool, Optional[str], List[Anomaly]]:
    anomalies: List[Anomaly] = []
    binary_exts = {
        ".exe",".dll",".so",".dylib",".bin",".app",".zip",".tar",".gz",".bz2",".xz",".7z",".rar",".jar",
        ".jpg",".jpeg",".png",".gif",".bmp",".tiff",".webp",".ico",".mp4",".avi",".mov",".wmv",".flv",
        ".mp3",".wav",".ogg",".pdf",".doc",".docx",".xls",".xlsx",".ppt",".pptx",".ttf",".otf",".woff",".woff2",
        ".db",".sqlite",".sqlite3",".pyc",".pyo",".class",".dex",
    }
    ...
    try:
        ...
        anomalies.append(Anomaly(path=str(path), blob_sha256="", typ=AnomalyType.READ_ERROR, severity=Severity.ERROR,
...
class _PoolManager:
    ...
    def get(cls) -> mp.pool.Pool:
        ...
        cls._ctx = ctx
        cls._pool = ctx.Pool(processes=LIMITS.POOL_PROCS, maxtasksperchild=LIMITS.POOL_MAX_TASKS_PER_CHILD)
        return cls._pool
...
def _worker_entry(args: Tuple[str, str, Dict[str, str], Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if _get_prebuilt_lang is not None:
        ...
        if bundle_env and Path(bundle_env).exists():
            ...
            for c in (Path.cwd() / "build" / "ts-langs.so", Path.cwd() / "build" / "ts-langs.dylib"):
                if Path(c).exists():
    ...
    for name in ("defs", "imports", "calls", "literals"):
        ...
        for b0, capname, node in tmp:
            ...
            captures.append({
                ...
                "line_start": l0, "col_start": c0,
                "line_end": l1, "col_end": c1,
...
def parse_file(abs_path: str, *, language: Language, blob_sha: str) -> ParseResult:
    ...
    if size_mb > LIMITS.MAX_FILE_MB:
        anomalies.append(Anomaly(path=str(path), blob_sha256="", typ=AnomalyType.FILE_TOO_LARGE,
                                 severity=Severity.ERROR, reason_detail=f"{size_mb:.1f}MB > {LIMITS.MAX_FILE_MB}MB"))
    ...
    if is_pathological:
        anomalies.append(Anomaly(path=str(path), blob_sha256="", typ=AnomalyType.PATHOLOGICAL_FILE,
                                 severity=Severity.WARN, reason_detail=",".join(indicators)))
    ...
    parse_timeout = min(LIMITS.BASE_PARSE_TIMEOUT_S + size_mb * LIMITS.TIMEOUT_PER_MB_S, LIMITS.MAX_PARSE_TIMEOUT_S)

    # Process pool execution
    pool = _PoolManager.get()
    lang_key = _lang_key(language)
    async_res = pool.apply_async(_worker_entry, args=((str(path), lang_key, qsrc, LIMITS.__dict__),))
    try:
        result: Dict[str, Any] = async_res.get(timeout=parse_timeout)
    except mp.context.TimeoutError:
        # nuke pool and rebuild to kill wedged worker
        _PoolManager.reset()
        anomalies.append(Anomaly(path=str(path), blob_sha256="", typ=AnomalyType.PARSE_TIMEOUT,
                                 severity=Severity.ERROR, reason_detail=f"Exceeded {parse_timeout:.1f}s"))
        metrics = ParseMetrics(parse_time_ms=int((time.perf_counter() - start) * 1000),
                               node_count=0, tree_bytes=size_bytes, cache_state="miss",
                               grammar_fp=grammar_fp, query_fp=query_fp)
        return ParseResult(status=ParseStatus.TIMEOUT, language=language, captures=[], anomalies=anomalies, metrics=metrics)
    except Exception as e:
        _PoolManager.reset()
    ...
    for c in raw_caps:
        captures.append(Capture(
            ...
            line_start=c["line_start"], col_start=c["col_start"],
            line_end=c["line_end"], col_end=c["col_end"],
    ...
    if len(captures) >= LIMITS.CAPTURE_LIMIT_GLOBAL:
        anomalies.append(Anomaly(path=str(path), blob_sha256="",
                                 typ=AnomalyType.CAPTURE_TRUNCATED, severity=Severity.WARN,
    ...
    res = ParseResult(status=status, language=language, captures=captures, anomalies=anomalies, metrics=metrics)
    _RESULT_CACHE.put(cache_key, res)
    return res

def update_file(abs_path: str, edits: List[dict], *, language: Language, prev_blob_sha: str, new_blob_sha: str) -> ParseResult:
    # For robustness & determinism at scale, we re-parse; the process pool + cache makes it cheap.
    try:
        # best-effort: drop prior cache entry
        qsrc = _load_query_sources(language)
        cache_key_old = (str(Path(abs_path)), prev_blob_sha, _grammar_fingerprint(language), _fingerprint_query_sources(qsrc))
...
def _error_result(language: Language, msg: str, t0: float) -> ParseResult:
    anomalies = [Anomaly(path="", blob_sha256="", typ=AnomalyType.PARSE_ERROR, severity=Severity.ERROR, reason_detail=msg)]
    metrics = ParseMetrics(parse_time_ms=int((time.perf_counter() - t0) * 1000),
                           node_count=0, tree_bytes=0, cache_state="error",
                           grammar_fp="unknown", query_fp="unknown")
    return ParseResult(status=ParseStatus.ERROR, language=language, captures=[], anomalies=anomalies, metrics=metrics)

def _terminal_result(status: ParseStatus, language: Language, anomalies: List[Anomaly], t0: float) -> ParseResult:
    metrics = ParseMetrics(parse_time_ms=int((time.perf_counter() - t0) * 1000),
                           node_count=0, tree_bytes=0, cache_state="miss",
                           grammar_fp="unknown", query_fp="unknown")
    return ParseResult(status=status, language=language, captures=[], anomalies=anomalies, metrics=metrics)
