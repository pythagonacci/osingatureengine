...
_ID_START = r"[^\W\d_]|_|$"
_ID_CONT = r"[^\W_]|_|$|\d"
IDENT_RE = re.compile(rf"({_ID_START})(({_ID_CONT})*)", re.UNICODE)
...
def _build_scope_tree(defs: list[DefSite]) -> None:
    order = sorted(
        range(len(defs)), key=lambda i: (defs[i].span.byte_start, -defs[i].span.byte_end)
    )
    stack: list[int] = []
    for i in order:
        cur = defs[i]
...
def _qname_chain(module_qname: str, defs: list[DefSite], idx: int | None, leaf_name: str) -> str:
    parts: list[str] = []
    j = idx
    while j is not None:
...
def _extract_py_def(s: str) -> tuple[str | None, int, tuple[str, ...], tuple[str, ...]]:
    flags: list[str] = []
    ss = s.strip()
...
def _normalize_param_list_python(param_block: str | None) -> tuple[int, tuple[str, ...]]:
    if not param_block:
        return 0, ()
...
def _extract_ts_js_def(s: str) -> tuple[str | None, int, tuple[str, ...], tuple[str, ...]]:
    ss = s.strip()
    flags: list[str] = []
...
def _coarse_type_token(t: str | None) -> str:
    if not t:
        return "?"
    x = t.strip()
    while "<" in x and ">" in x:
...
def _normalize_param_list_ts(param_block: str | None) -> tuple[int, tuple[str, ...]]:
    if not param_block:
        return 0, ()
    inner = param_block.strip()[1:-1] if param_block.startswith("(") else param_block
...
def lift_to_ucg(
    ...
    *,
    ...
    module_node_id = _node_id(
        SemanticType.MODULE, language, module_qname, _span_key(abs_path, 0, 0)
    )
    ...
    edges.append(
        _mk_edge(
            ...
            file_node_id,
            module_node_id,
            Span(abs_path, 0, 0, 0, 0, 0, 0, "seed:defines"),
        )
    )
    ...
    for i, d in enumerate(def_sites):
        ...
        qname = (
            _qname_chain(module_qname, def_sites, d.parent_idx, leaf)
            if d.parent_idx is not None
    ...
    decl_spans = sorted(
        [(d.span, d.node_id) for d in def_sites], key=lambda x: (x[0].byte_start, -x[0].byte_end)
    )

    # 4) Group & materialize IMPORT/EXPORT/CALL + placeholders
    import_caps: list[Capture] = [
        c
        for c in parse.captures
        if CAPTURE_KIND_TO_SEM.get(c.kind, (None, ""))[0] == SemanticType.IMPORT
    ]
    export_caps: list[Capture] = [
        c
        for c in parse.captures
        if CAPTURE_KIND_TO_SEM.get(c.kind, (None, ""))[0] == SemanticType.EXPORT
    ]
    call_caps: list[Capture] = [
        c
        for c in parse.captures
        if CAPTURE_KIND_TO_SEM.get(c.kind, (None, ""))[0] == SemanticType.CALL
    ]
    sym_caps: list[Capture] = [
        c
        for c in parse.captures
        if CAPTURE_KIND_TO_SEM.get(c.kind, (None, ""))[0] == SemanticType.SYMBOL
    ]
    lit_caps: list[Capture] = [
        c
        for c in parse.captures
        if CAPTURE_KIND_TO_SEM.get(c.kind, (None, ""))[0]
        in (SemanticType.LITERAL, SemanticType.TEMPLATE_PART)
    ]
    ...
    def _group(caps: list[Capture]) -> list[tuple[Span, list[Capture]]]:
        ...
        for c in cs:
            if not buf:
                buf = [c]
                b0, b1 = c.byte_start, c.byte_end
                continue
            if c.byte_start <= b1:  # overlap → same statement
                buf.append(c)
                b1 = max(b1, c.byte_end)
            else:
                groups.append(
                    (
                        Span(
    ...
    for gspan, gcaps in _group(import_caps + [c for c in sym_caps if c.kind.startswith("import.")]):
        ...
        nodes.append(
            UCGNode(
                node_id=import_id,
                semantic_type=SemanticType.IMPORT,
                raw_type=";".join(sorted({c.kind for c in gcaps})),
                name=None,
                qualified_name=q,
                language=language,
                spans=[gspan],
        ...
        for alias in _infer_import_aliases(gcaps, language):
            ...
            sym_node = UCGNode(
                node_id=sid,
                semantic_type=SemanticType.SYMBOL,
                raw_type="import.symbol",
                name=alias,
                qualified_name=sym_q,
                language=language,
                spans=[gspan],
                extra={},
            )
            ...
            edges.append(
                UCGEdge(
                    ...
                    confidence=0.4,
                    spans=[gspan],
                    reason_label="normalize:import.binds",
                )
            )
    ...
    for gspan, gcaps in _group(export_caps):
        ...
        nodes.append(
            UCGNode(
                node_id=export_id,
                semantic_type=SemanticType.EXPORT,
                raw_type=";".join(sorted({c.kind for c in gcaps})),
                name=None,
                qualified_name=q,
                language=language,
    ...
    for gspan, gcaps in _group(call_caps + [c for c in sym_caps if c.kind.startswith("call.")]):
        ...
        nodes.append(
            UCGNode(
                node_id=call_id,
                semantic_type=SemanticType.CALL,
                raw_type=";".join(sorted({c.kind for c in gcaps})),
                name=None,
                qualified_name=q,
                language=language,
        ...
        if caller:
            edges.append(
                UCGEdge(
                    ...
                    confidence=1.0,
                    spans=[gspan],
                    reason_label="normalize:callsite",
                )
            )
    ...
    nodes.sort(
        key=lambda n: (
            n.language.value,
            n.semantic_type.value,
            n.qualified_name or "",
            n.spans[0].byte_start,
        )
    )
    ...
    return LiftResult(
        nodes=nodes, edges=edges, symbols=symbols, anomalies=anomalies, metrics=metrics
    )
...
def _cap_span(path: str, cap: Capture) -> Span:
    return Span(
        ...
        line_start=cap.line_start,
        col_start=cap.col_start,
        line_end=cap.line_end,
        col_end=cap.col_end,
...
def _infer_module_qname(path: str, module_root: str | None) -> str:
    p = Path(path)
    if module_root:
        try:
...
def _extract_name_and_sig(
    language: Language, sem: SemanticType, preview: str
) -> tuple[str | None, SigNorm | None]:
...
def _nearest_enclosing_decl(decl_spans: list[tuple[Span, str]], b0: int, b1: int) -> str | None:
    # decl_spans sorted by start asc, end desc
    best: tuple[int, str] | None = None  # (area, node_id)
...
def _infer_callee_name_from_group(caps: list[Capture]) -> str | None:
